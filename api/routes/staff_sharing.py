...
def _coach_notify(db: Session, coach_id: int, title: str, body: str, ref_id: int | None = None,
    ...
    if event:
        ...
        _after_commit(db, lambda: notify.coach_event(recipient, event, {
            ...
            # Handed over whole. Swapping the underscores for spaces used to
            # be the whole of the translation here, which meant the reader got
            # an English type however their app was set, and once a title was
            # attached they got the join character too.
            "report": p.get("type") or "report",
        }))
    elif key:
...
def share_with_staff(
    ...
    ensure_teams_for_share(db, body.report_type, body.report_id, coach, body.recipient_id)
    ...
    _coach_notify(
        ...
        key="notifs.staffReportShared", params={"coach": coach.name,
                "type": report_titles.qualified_type(body.report_type, body.report_id, db)},
    )
    db.commit()
    db.refresh(sr)
    ...
    for rid in ids:
        ...
        _coach_notify(
            ...
            params={"coach": coach.name, "team": team.name,
                    "type": report_titles.qualified_type(report_type, report_id, db)},
        )
        count += 1
    db.commit()
    ...
    for other_id in others:
        _coach_notify(
            ...
            params={"coach": coach.name, "text": body.text[:120],
                    ...
                    # the next one. The kind alone could not do that: a coach
                    # with three shared evaluations read "commented on Player
                    # Eval" three times and had to open all three.
                    "item": report_titles.qualified_type(
                        sr.report_type, sr.report_id, db)},
        )
    db.commit()
    db.refresh(comment)
...
async def regenerate_shared(
    ...
    _coach_notify(
        ...
        key="notifs.staffReportRegenerated", params={"coach": coach.name,
                "type": report_titles.qualified_type(sr.report_type, sr.report_id, db)},
    )
    db.commit()
    db.refresh(sr)
    return _build_out(sr, db)
...
def request_updated(
    ...
    _coach_notify(
        ...
        key="notifs.staffReportRequest", params={"coach": coach.name,
                "type": report_titles.qualified_type(sr.report_type, sr.report_id, db)},
    )
...
def respond_request(
    ...
    if not approve:
        ...
        key="notifs.staffReportDeclined", params={"coach": coach.name,
                "type": report_titles.qualified_type(sr.report_type, sr.report_id, db)},
        )
        db.commit()
    ...
    _coach_notify(
        ...
        key="notifs.staffReportUpdatedShared", params={"coach": coach.name,
                "type": report_titles.qualified_type(sr.report_type, sr.report_id, db)},
    )
    db.commit()
...
def forward_shared(
    ...
    _coach_notify(
        ...
        ref_id=new_sr.id,
        ntype="staff_report_shared",
        key="notifs.staffReportForwarded", params={"coach": coach.name,
                "type": report_titles.qualified_type(sr.report_type, sr.report_id, db)},
    )
    db.commit()
    db.refresh(new_sr)
